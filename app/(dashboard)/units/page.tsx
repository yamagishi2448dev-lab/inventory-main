...
export default function UnitsPage() {
    ...
    return (
        <div className="space-y-6">
            ...
            <Card>
                <CardHeader>
                    <CardTitle>新規単位</CardTitle>
                </CardHeader>
                <CardContent>
                    <form onSubmit={handleCreate} className="flex gap-4">
                        <div className="flex-1">
                            <Label htmlFor="newName" className="sr-only">単位名</Label>
                            <Input
                                id="newName"
                                value={newName}
                                onChange={(e) => setNewName(e.target.value)}
                                placeholder="単位名を入力（例: 台、個）"
                                maxLength={50}
                            />
                        </div>
                                ...
                                units.map((unit) => (
                                    <TableRow key={unit.id}>
                                        ...
                                        <TableCell className="text-right space-x-2">
                                            {editingId === unit.id ? (
                                                <>
                                                    <Button size="sm" onClick={() => handleUpdate(unit.id)} disabled={saving}>
                                                        保存
                                                    </Button>
