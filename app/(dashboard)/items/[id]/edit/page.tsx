...
import type { ItemWithRelations, ItemType } from '@/lib/types'
...
export function ItemEditPageCore({
  ...
  useEffect(() => {
    ...
    const fetchItem = async () => {
      try {
        ...
        if (item.images) {
          setImages(item.images.map((img) => ({ url: img.url, order: img.order })))
        }
        if (item.materials) {
        ...
        if (item.tags) {
          setSelectedTagIds(item.tags.map((t) => t.tagId || t.tag?.id || '').filter(Boolean))
        }
      } catch (err) {
  ...
  const handleSubmit = async (e?: React.FormEvent | React.MouseEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || 'アイテムの更新に失敗しました')
      }
      ...
      const materialsResponse = await fetch(`/api/items/${itemId}/materials`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      const typeLabel = itemType === 'PRODUCT' ? '商品' : '委託品'
      alert(`${typeLabel}を更新しました`)
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <h1 className="text-3xl font-bold">{getTypeLabel()}編集</h1>
        <Badge variant={itemType === 'PRODUCT' ? 'default' : 'secondary'}>
          {getTypeLabel()}
        </Badge>
      </div>
      <p className="text-gray-500">SKU: <span className="font-mono">{itemSku}</span></p>
