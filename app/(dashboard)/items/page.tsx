...
import { DEFAULT_PAGE_SIZE, PRINT_SELECTION_STORAGE_KEY, ITEM_SELECTION_STORAGE_KEY } from '@/lib/constants'
...
export default function ItemsPage() {
  ...
  const itemType: ItemType | undefined = typeParam === 'PRODUCT' || typeParam === 'CONSIGNMENT' ? typeParam : undefined
  ...
  }, [itemType, search, categoryId, manufacturerId, locationId, tagIds, includeSold, sortBy, sortOrder])
  ...
  useEffect(() => {
    const rawSelection = sessionStorage.getItem(ITEM_SELECTION_STORAGE_KEY)
    if (!rawSelection) {
      return
    }
    ...
    try {
      ...
      sessionStorage.removeItem(ITEM_SELECTION_STORAGE_KEY)
    }
  }, [])
  ...
  useEffect(() => {
    ...
    sessionStorage.setItem(ITEM_SELECTION_STORAGE_KEY, JSON.stringify(ids))
  }, [selectedItemIds])
  ...
  const handleBulkEdit = async (updates: {
    ...
    try {
      setBulkOperationLoading(true)
      const response = await fetch('/api/items/bulk/edit', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  }, [router, pathname, viewMode, itemType, handleClearSelection])
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:gap-4 lg:flex-1 lg:justify-center">
            ...
            <div className="relative w-full sm:w-64">
              ...
              <Input
                type="text"
                placeholder={`${getTypeLabel()}名、仕様で検索...`}
                value={searchInput}
          ...
          <div className="flex flex-wrap items-center gap-2 lg:justify-end">
            ...
            <div className="hidden sm:flex flex-wrap items-center gap-2">
              ...
              <a href="/api/items/import/template" download>
              ...
              <a href={`/api/items/export${itemType ? `?type=${itemType}` : ''}`} download>
                <Button variant="outline" size="sm" className="h-8 text-xs">
            ...
            <Link href={`/items/new${itemType ? `?type=${itemType}` : ''}`}>
                  ...
                  <TableRow className="bg-slate-100/70">
                    ...
                    <TableHead className="py-1.5 px-2">
                      <SortableHeader field="name">
                        <span className="text-xs font-medium">{getTypeLabel()}名</span>
                      </SortableHeader>
                    </TableHead>
                    <TableHead className="py-1.5 px-2">
