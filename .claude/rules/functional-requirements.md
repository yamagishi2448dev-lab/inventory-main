...
- 表示列（v2.0）: メーカー/品目/商品名/仕様/張地/カラー/個数/原価単価/場所/タグ
- 写真ビュー: 2列の大カード表示
- 検索: 商品名部分一致（SKU完全一致は旧仕様）
- リアルタイム検索: 300msデバウンス
- フィルタ: manufacturerId, categoryId, locationId, tagIds, arrivalDate
- タグフィルタ: 複数選択時はOR条件（いずれかのタグを持つ商品を表示）
- 選択: 行/全件チェック、フィルタ結果の全件選択（ページ外含む）
- 詳細: 全フィールド + 画像ギャラリー + タグ表示
- 画像: 追加/削除/並び替え可能
- 在庫数の変更履歴は将来機能
- 削除: 物理削除（画像はCascade）
- 販売済み管理: isSold, soldAt

## Consignment Management（委託品）
- 商品管理と同等の機能
- SKU形式: CSG-XXXXX
- 原価単価は常に0

## Tag Management（v2.2追加）
- タグのCRUD
- 商品・委託品への複数タグ付与
- タグでフィルタリング（マルチセレクト、OR条件）
- タグ一覧に商品数・委託品数を表示
- 削除時はProductTag/ConsignmentTagもCascade削除

## Bulk Operations（一括操作）
- 複数商品/委託品の一括選択
- 一括編集: 場所、タグなどを一括変更
- 一括削除: 選択した商品/委託品を一括削除

## CSV Import/Export（v2.2追加）
- 商品/委託品のCSVエクスポート
  - タグはカンマ区切りで出力
  - フィルタ条件を反映
- 商品/委託品のCSVインポート
  - テンプレートダウンロード機能
  - タグはカンマ区切りで入力（存在しないタグは自動作成）
  - バリデーションエラーは行番号と共に表示
...
- 形式: JPEG/PNG/WebP
- 最大: 5MB、5枚/商品
- 保存先: 開発環境は`public/uploads`、本番環境はGoogle Drive

## Master Data
- メーカー/品目/場所/単位/タグのCRUD
- 一覧に商品数を表示
- 削除時はProductの参照をSetNull（タグは例外、Cascade削除）
