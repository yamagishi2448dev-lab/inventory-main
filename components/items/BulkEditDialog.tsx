...
export function BulkEditDialog({
    ...
    const [tagChangeMode, setTagChangeMode] = useState<boolean>(false)

    // showCostPrice は現在の実装では使用していないが、将来の拡張のために保持
    void showCostPrice
    ...
    return (
        <Dialog open={open} onOpenChange={handleClose}>
            <DialogContent className="sm:max-w-[500px]">
                <DialogHeader>
                    <DialogTitle>アイテムを一括編集</DialogTitle>
                    <DialogDescription>
                        選択した{selectedCount}件のアイテムを編集します。変更したいフィールドのみ選択してください。
                    </DialogDescription>
                </DialogHeader>
                ...
                <div className="grid gap-4 py-4">
                    ...
                    <div className="grid gap-2">
                        ...
                        {tagChangeMode && (
                            <p className="text-xs text-muted-foreground">
                                選択したタグが全てのアイテムに設定されます（既存のタグは上書きされます）
                            </p>
                        )}
                    </div>
                </div>
