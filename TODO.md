...
#### 認証漏れの是正（優先）
- [ ] 在庫更新APIに認証を追加（`app/api/products/[id]/stock/route.ts`）。少なくとも認証済みユーザーに限定し、必要に応じて権限を確認。
- [ ] 商品画像削除APIに認証・所有確認を追加（`app/api/products/[id]/images/[imageId]/route.ts`）。紐づく商品に対する権限を検証。
- [ ] 画像アップロードAPIを認証付き＋レート制御に変更（`app/api/upload/route.ts`）。ストレージ悪用防止のため拡張子/サイズ/頻度の制限強化。
- [ ] 商品CSVエクスポートAPIを認証（必要ならADMIN限定）で保護（`app/api/products/export/route.ts`）。原価・場所等の機微情報漏えい防止。
- [ ] カテゴリ詳細/更新/削除APIに認証を追加し一覧/作成と整合（`app/api/categories/[id]/route.ts`）。
- [ ] その他の下位リソースAPIを横断チェックし、認証漏れがないか確認。
...
#### サイドバーの修正
- [x] サイドバーのスクロール問題を解決
  - デスクトップ時にサイドバーを画面に固定（`sticky top-0 h-screen`）
  - サイドバー内で独立してスクロール可能に（`overflow-y-auto`）
  - モバイルの動作は変更なし（既存の`fixed`を維持）
- [x] サイドバー下部のログアウトボタンを削除
  - ログアウトは右上のユーザーメニューからのみ可能に統一
  - `LogOut`アイコンと`handleLogout`関数を削除
...
- [x] セッションはHttpOnly Cookieで管理
- [x] 入力値はZodで検証
- [x] XSS対策（Reactの標準エスケープ）
